...
4. Auth → Providers → Email: enable. Then **disable "Confirm email"** for Phase 0
   (Auth → Providers → Email → turn OFF "Confirm email"). This is required: with
   it on, `signUp()` returns no session, so the signup → verify-phone → discover
   flow dead-ends at /sign-in. (Phone provider stays OFF in Phase 0 — we stub OTP
   in dev. Re-enable email confirmation in a later phase with a proper callback.)
